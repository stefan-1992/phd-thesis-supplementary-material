...
EXPERIMENT_NAMES = ['gen_data', 'add_data_150', 'add_data_855']
...
if __name__ == '__main__':
    ...
    for exp in args.exp.split(','):
        ...
        from egice_training import init_tokenizer, train, transform_lists_to_ds
        ...
        for split in [RANDOM_SPLIT, DOC_SPLIT]:
            for seed in [int(i) for i in args.seed.split(',')]: 
                ...
                df = pd.read_csv('data/lrml_v8_' + EXPERIMENT_NAMES[exp] + '.csv')

                df['input'] = 'translate English to LegalRuleML: ' + \
                    get_file_names_from_df(df) + ' ' + df['text']
                df['label'] = df['spaced_lrml']
                ...
                hyperparameters = dict(epochs=args.epochs, lr=lr, bs=bs, adafactor=False, weight_decay=1.000e-1, metric_for_best_model=metric_for_best_model,
                                       ...
                                       fp16=False, split=split, experiment=EXPERIMENT_NAMES[exp], exp_num=exp, calc_loss=True, pred_epoch=args.pred_epoch)
