...
def get_datasets():
    ...
    silver_dataset = transform_lists_to_ds((prefix + text).tolist(), lrml.tolist(), max_length=512)
    text = lrml_df['text']
    lrml = lrml_df['lrml']
    base_original_512_ds = get_splits(prefix + text, lrml, shuffle=False, max_length=512)

    return {'train': silver_dataset, 'test': base_original_512_ds['test']}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='silver',
    ...
    bs = args.batchsize
    dr = args.decoding_runs
    drp = args.decoding_runs_prediction if args.decoding_runs_prediction else dr
        ...
        for lr in lrs:
            for bs in bss:
                ...
                hyperparameters = dict(epochs=args.epochs, learning_rate=lr, batch_size=bs, adafactor=False,
                                       ...
                                       seed=seed, decoding_runs=dr, decoding_runs_prediction=drp,
                                       prefix_list=prefix_list, metric_names=metric_names,
                                       write_predictions=args.write_predictions, evaluate_only=args.evaluate_only,
